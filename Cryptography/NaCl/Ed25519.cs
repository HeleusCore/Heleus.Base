...
namespace Chaos.NaCl.Cryptography
{
    public static class Ed25519
    {
        ...
        public static void KeyPairFromSeed(out byte[] publicKey, out byte[] expandedPrivateKey)
        {
            var seed = new byte[PrivateKeySeedSizeInBytes];
            using(var r = RandomNumberGenerator.Create())
                r.GetBytes(seed);

            KeyPairFromSeed(out publicKey, out expandedPrivateKey, seed);
        }
        ...
        public static void KeyPairFromSeed(ArraySegment<byte> publicKey, ArraySegment<byte> expandedPrivateKey)
        {
            ...
            using (var r = RandomNumberGenerator.Create())
                r.GetBytes(seed);
            KeyPairFromSeed(publicKey, expandedPrivateKey, new ArraySegment<byte>(seed));
        }
