...
namespace Heleus.Cryptography.Scrypt
{
    ...
    public static class ScryptEncoder
    {
        ...
        const int DefaultIterationCount = 16384 / 2;
        ...
        public static ScryptResult Encode(byte[] password)
        {
            return Encode(password, DefaultIterationCount, DefaultBlockSize, DefaultThreadCount, ScryptSaltGenerator.Default);
        }

        public static ScryptResult Encode(byte[] password, byte[] saltBytes)
        {
            return Encode(password, DefaultIterationCount, DefaultBlockSize, DefaultThreadCount, saltBytes);
        }

        public static ScryptResult Encode(byte[] password, int iterationCount, int blockSize, int threadCount)
        {
            return Encode(password, iterationCount, blockSize, threadCount, ScryptSaltGenerator.Default);
        }
        ...
        public static ScryptResult Encode(byte[] password, int iterationCount, int blockSize, int threadCount, byte[] saltBytes)
        {
            if (iterationCount < 1)
            {
            ...
            if (saltBytes == null || saltBytes.Length != SaltLength)
            {
                throw new ArgumentOutOfRangeException(nameof(saltBytes), $"SaltBytes must have a size of {SaltLength}.");
            }
            ...
            return EncodeV2(password, saltBytes, iterationCount, blockSize, threadCount);
        }

        /// <summary>
        /// Checks if the given hash is a valid scrypt hash
        /// </summary>
        public static bool IsValid(string hashedPassword)
        {
        ...
        /// </summary>
        private static ScryptResult EncodeV2(byte[] password, byte[] saltBytes, int iterationCount, int blockSize, int threadCount)
        {
            ...
            if (N <= 1 || (N & (N - 1)) != 0)
            {
                throw new ArgumentException("iterationCount must be a power of two greater than 1", nameof(iterationCount));
            }
            ...
            var hashed = CryptoScrypt(password, saltBytes, N, r, p);

            return new ScryptResult(iterationCount, blockSize, threadCount, saltBytes, hashed);
        }
